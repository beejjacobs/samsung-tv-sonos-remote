...
class SamsungRemote {
  ...
  sendToDevice(command) {
    ...
    socket.onClose = (hasError) => {
      this.log('onClose', hasError);
    };
    this.log(this.ip, this.port);
    socket.open(this.ip, this.port,
        async () => {
          let message = this.buildMessage(command);
          try {
            await SamsungRemote.write(socket, message.first);
            await SamsungRemote.write(socket, message.second);
            socket.shutdownWrite();
          } catch (e) {
            this.error('write error', e);
          }
  ...
  buildMessage(command) {
    ...
    this.log({ipEncoded, macEncoded, hostNameEncoded});
  ...
  log(...args) {
    if (this.debug) {
      console.log('[SamsungRemote]', ...args);
    }
  }
  ...
  error(...args) {
    if (this.debug) {
      console.error('[SamsungRemote]', ...args);
    }
  }
